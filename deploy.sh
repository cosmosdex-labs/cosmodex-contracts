...
cp -R .stellar/* ../UI/cosmoUI/.stellar/
...
stellar contract bindings typescript \
  ...
  --output-dir ../UI/cosmoUI/packages/PoolFactory --overwrite

stellar contract bindings typescript \
  --network testnet \
  --contract-id $token_launcher_contract_id \
  --output-dir ../UI/cosmoUI/packages/TokenLauncher --overwrite

stellar contract bindings typescript \
  --network testnet \
  --contract-id $usdt_token_contract_id \
  --output-dir ../UI/cosmoUI/packages/USDTToken --overwrite

stellar contract bindings typescript \
  --network testnet \
  --contract-id $sample_pool_contract_id \
  --output-dir ../UI/cosmoUI/packages/Pool --overwrite

# Create deployment.ts file
log "Creating deployment.ts file..."
DEPLOYMENT_TS_PATH="../UI/cosmoUI/packages/deployment.ts"
...
cd ../UI/cosmoUI/packages/PoolFactory
npm install --force
npm run build
